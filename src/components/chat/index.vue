<!--
 * @Author: your name
 * @Date: 2020-04-18 10:58:08
 * @LastEditTime: 2020-04-20 23:10:41
 * @LastEditors: Please set LastEditors
 * @Description: 左侧列消息表
 * @FilePath: \customChat\src\components\chat\index.vue
 -->
...
<template>
  <div class="userlist">
    <a-menu style="width: 100%; border-right: 0;" mode="vertical" :selectedKeys="selectedKeys">
      ...
      >
        <span v-if="item.groupname" class="custom-title">{{item.name}}</span>
        <span v-else class="custom-title">
          <img :src="item.Header" width="35" height="35" alt />
          {{item.username}}
        </span>
        <!-- 未读信息 -->
...
export default {
  ...
  computed: {
    ...
    userList() {
      console.log(this.contact, this.group, this.chatroom, "2222");
      return {
