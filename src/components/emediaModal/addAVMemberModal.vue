<template>
  <a-modal title="选择成员" :visible="addAVMemberModalVisible" @ok="startConference" @cancel="onClosed">
    <div>
      <el-checkbox-group v-model="checkList" class="checkboxGroup" :max="6">
        <div
          style="border-bottom: 1px solid #f2f2f2; height: 30px"
...
export default {
  ...
  methods: {
    ...
    startConference() {
      ...
      emedia.mgr
        .createConference(
          emedia.mgr.ConfrType.COMMUNICATION_MIX,
          pwd,
          rec,
          recMerge
        )
        .then(function(confr) {
          ...
          WebIM.EMService.joined(confr.confrId) ||
            WebIM.EMService.joinConferenceWithTicket(
              confr.confrId,
              tkt,
              "user ext field"
            )
              .then(function() {
                WebIM.EMService.publish(
