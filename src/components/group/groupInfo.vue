        ...
        <div class="info-user">
          <div>
            ...
            >
              <span
                v-if="!item.owner"
                :style="{cursor:item.Header?'pointer':'not-allowed'}"
                class="info-name"
                @click="item.Header?newChat(item):''"
              >
                <img
                  :src="item.Header?item.Header:require('../../assets/kefu.png')"
                  alt
                  width="35"
                  height="35"
                />
                {{item.name}} {{item.member}}
              </span>

              <span
                v-if="!item.owner&&(adminList.includes(username) || groupinfoList.admin == username)&&username != item.member"
                class="info-icon"
              >
                <!-- 设置管理员 -->
                <el-tooltip
            ...
            <div class="listItem" @click="newsChat(groupinfoList.members[1])">
              <img :src="groupinfoList.members[1].Header" alt width="35" height="35" />
              {{groupinfoList.members[1].name}} {{groupinfoList.admin}}
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
     ...
     */
    newChat(item) {
      console.log(item, "item", this.$store.state.chat.userList, this.$data);

      let userList = this.$store.state.chat.userList.contactUserList;
      userList.push({
        chatid: item.member,
        name: item.member,
      ...
      this.$parent.onEmitParentFuntion({
        key: "contact"
      });
    ...
    chengeInfoModel() {
      ...
      setTimeout(() => {
        ...
        if (
          this.adminList.includes(this.username) ||
          this.groupinfoList.admin == this.username
        ) {
      ...
      this.$confirm("确认操作: 设为管理员", {
        ...
        .then(() => {
          this.onSetAdmin({
            ...
            success: function() {
              me.getGroupAdmin({
                select_id: me.$store.state.group.groupInfo.gid
              });
              me.$forceUpdate();
            }
          });
