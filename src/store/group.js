...
const Group = {
  ...
  actions: {
    ...
    onGetGroupinfo: function(context, payload) {
      ...
      let list = [];
      Api.findUserGroupDetail({ groupId: gid }).then(res => {
        list = res.data;
        let options = {
          groupId: gid, //群组id
          success: function(resp) {
            resp.data[0].affiliations[0] = {
              ...resp.data[0].affiliations[0],
              Header: "",
              account: res.data.account,
              name: res.data.nickName
            };
            resp.data[0].affiliations[2] = {
              ...resp.data[0].affiliations[2],
              Header: res.data.doctorUserHead,
              account: res.data.doctorAccount,
              name: res.data.doctorUserName
            };
            resp.data[0].affiliations[1] = {
              ...resp.data[0].affiliations[1],
              Header: res.data.userHead,
        ...
        success: function(resp) {
          if (Vue.$store.state.group.groupInfo.membersonly != true) {
            ...
            Message({
              ...
              message:
                "已申请加入群组：" +
                Vue.$store.state.group.groupInfo.gid +
                "，等待管理员同意"
            });
          }
        },
