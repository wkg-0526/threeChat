...
WebIM.conn.listen({
  onOpened: function(message) {
    ...
    // 登录或注册成功后 跳转到好友页面
    const username =
      Vue.$store.state.login.username ||
    ...
    Vue.$router.push({ path: path, query: { username } });
  ...
  onTextMessage: function(message) {
    ...
    // 判断当前type是contact 更新userList
    if (type === "chat") {
      let userList = Vue.$store.state.chat.userList.contactUserList;

      let arr = [];
      console.log(message, 22);
      Api.findUserAppAccount({ appAccount: message.from }).then(res => {
        console.log(res);
        if (userList.length === 0) {
          userList.push({
            ...
            Header: res.data.list[0].userHead
          });
        } else {
          let isExist = userList.some(item => {
            return item.chatid === message.from;
          });
          console.log(isExist);
          if (isExist === false) {
            userList.push({
            ...
            console.log(userList, "987");
            // userList.splice(0, 1, userList.splice(userList.length - 1, 1));
          }
        }
        Vue.$store.commit("updateUserList", {
          userList,
